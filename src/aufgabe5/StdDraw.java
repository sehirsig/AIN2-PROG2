...
*/
        ...
        StdDraw.show(0);
        draw(0.0D, 3.0D, 0.0D, 0.8D, 0.01D);
        StdDraw.show(0);

/*
        StdDraw.setXscale(0, 5);
        StdDraw.setYscale(0, 5);
        double dd = Math.random() * (60 - 30) + 30.0D;
        StdDraw.show(0);
        draw2(dd, 0.0D, 2.0D, 1.0D, 1.0D, 0.4D, 0.0D, 0.01D);
        StdDraw.show(0);*/
    }

    public static void draw(double delta2, double xa, double ya, double w, double d) {
        if (w >= 0.01D) {
            delta2 = Math.toRadians(delta2);
            StdDraw.setPenRadius(d);
            double c = w * Math.cos(delta2);
            double s = w * Math.sin(delta2);
            double u = w * Math.cos(Math.toRadians(30.0D));
            double v = w * Math.sin(Math.toRadians(30.0D));
            double xb = xa + c;
            double yb = ya + s;
            double xc = xa + c - s;
            double yc = ya + s + c;
            double xd = xa - s;
            double yd = ya + c;
            double xe = xd + u * Math.cos(Math.toRadians(30.0D) + delta2);
            double ye = yd + u * Math.sin(Math.toRadians(30.0D) + delta2);
            if (w < 0.1D) {
            ...
            delta2 = Math.toDegrees(delta2);
            draw(30.0D + delta2, xd, yd, u, d * 0.82D);
            draw(30.0D + delta2 - 90.0D, xe, ye, v, d * 0.82D);
        }
    ...
    public static void draw2(double delta, double delta2, double xa, double ya, double len, double w, double i, double d) {
        if (w >= 0.008D) {
            ...
            double c2 = len * Math.cos(delta2);
            double s2 = len * Math.sin(delta2);
            double xb = xa + c;
            double yb = ya + s;
            double xd = xa - s2;
            double yd = ya + c2;
            double xc = xd + c;
            double yc = yd + s;
            double xe = xd + u * Math.cos(Math.toRadians(delta) + delta2);
            double ye = yd + u * Math.sin(Math.toRadians(delta) + delta2);
            if (w < 0.03D) {
                StdDraw.setPenColor(StdDraw.GREEN);
            } else {
            ...
            StdDraw.line(xb, yb, xc, yc);
            StdDraw.line(xd, yd, xa, ya);
            len = Math.random() * (0.8 - 0.2) + 0.2D;
            if (len > w * 2.0D) {
                System.out.println(123);
                len = w * 2.0D;
            }
            double len2 = Math.random() * (0.8 - 0.2) + 0.2D;
            if (len2 > w * 2.0D) {
                System.out.println(123);
                len2 = w * 2.0D;
            }
            ...
            delta2 = Math.toDegrees(delta2);
            delta = (Math.random() * (50 - 40) + 40.0D); // * Math.pow(-1, (int)(Math.random() * 10))
            ++i;
            draw2(delta, delta + delta2, xd, yd, len, u, i, d * 0.78D);
            draw2(delta, delta + delta2 - 90.0D, xe, ye, len2, v, i, d * 0.78D);
        }
    ...
    private static void drawquadrat(double Ax, double Ay, double laenge, double winkel){
        if (laenge == 2.0) {
            double x = Ax;
            double y = Ay;
            double w = laenge;
            double winkely = 0;
            double s = w * Math.sin(Math.toRadians(winkely));
            double c = w * Math.cos(Math.toRadians(winkely));
            double Bx = x + c;
            double By = y + s;
            double Cx = x + c - s;
            double Cy = y + s + c;
            ...
            double u = w * Math.cos(Math.toRadians(30));
            double v = w * Math.sin(Math.toRadians(30));
            double Ex = x - s + u * Math.cos(Math.toRadians(winkely + 30));
            double Ey = y + c + u * Math.sin(Math.toRadians(winkely + 30));

            drawquadrat(Dx, Dy, u, winkely + 30);
            ...
            double ky = By;
            double kw = v *1.1;
            double kwinkely = winkel - 90;
            double ks = kw * Math.sin(Math.toRadians(kwinkely));
            double kc = kw * Math.cos(Math.toRadians(kwinkely));
            double kBx = kx + kc;
            double kBy = ky + ks;
            double kCx = kx + kc - ks;
            double kCy = ky + ks + kc;
            double kDx = kx - ks;
            double kDy = ky + kc;
            double ku = kw * Math.cos(Math.toRadians(30));

            //StdDraw.line(kx, ky, kBx, kBy);
            StdDraw.line(kBx, kBy, kCx, kCy);
            //StdDraw.line(kCx, kCy, kDx, kDy);
            StdDraw.line(kDx, kDy, kx, ky);




            drawquadrat(kDx, kDy, ku, winkel - 60);
            drawquadrat(Dx, Dy, u, winkel + 30);
    ...
    public static void variante2(double Ax, double Ay, double laenge) {
